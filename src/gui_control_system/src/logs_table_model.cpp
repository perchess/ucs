...
QVariant LogsTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole)
    {
        ...
        {
        case 0:
            return QString::number(log.count);
        case 1:
        ...
        case 2:
        {
            ...
            break;
        case 3:
            return QString::fromStdString( log.node.get() );
        case 4:
            return log.message;
        case 5:
            return QString::fromStdString( log.source.get() );
        default:
            break;
        }
        return QVariant();
    ...
    else if( role== Qt::ForegroundRole)
    {
        switch( log.level_raw )
        {
        case DEBUG:    return QColor::fromRgb(50,  50 , 50) ;  // black
        case INFO:     return QColor::fromRgb(0,   0  , 255);  // blue
        case WARNINGS: return QColor::fromRgb(240, 120, 0);    // orange
        case ERROR:    return QColor::fromRgb(255, 0  , 0);    // red
        default: break;
        }
    }
    else if( role == Qt::UserRole)
    {
        switch( index.column() )
        {
        case 0: return QString::number(log.count);
...
QHash<int, QByteArray> LogsTableModel::roleNames() const
{
